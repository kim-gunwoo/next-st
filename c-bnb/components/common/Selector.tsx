...
const Container = styled.div<SelectorContainerProps>`
  ...
  select {
    ${({ validateMode, isValid }) => {
      if (validateMode) {
        if (!isValid) {
          return css`
            border-color: ${({ theme }) => theme.tawny};
    ...
    &:disabled {
      ...
      border-color: ${({ theme }) => theme.gray_e5};
      color: ${({ theme }) => theme.gray_e5};
      cursor: not-allowed;
    }
  }
  ...
  .selector-warning {
    ...
    p {
      font-size: 12px;
      color: ${({ theme }) => theme.davidson_orange};
    }
  }
`;
...
const RegisterSelectorStyle = css`
  ...
  select {
    ...
    height: 56px;
    border-radius: 8px;
    border: 1px solid ${({ theme }) => theme.gray_b0};
